        ...
        <v-card class="pa-12 pb-8" elevation="8" width="600" rounded="lg">
            <form @submit.prevent="login">
                <div class="d-flex justify-center my-2 mb-6">
                    <v-icon @click="switchTheme" :class="{ 'text-white': dark }" style="font-size:64px;"
                        icon="mdi-airplane" />
                </div>
                ...
                <v-btn type="submit" block class="mb-8
                    font-weight-bold" :color="dark ? 'white' : 'dark'" size="large" variant="tonal" :disabled="loading" :loading="loading">
                    Entrar
                </v-btn>
            </form>
        </v-card>
